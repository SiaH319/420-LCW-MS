...
Assignment 2
'''
# THIS IS ONLY FOR THE EX2; ITS CODE IS ALMOST THE SAME AS THE CODE FOR EX1
...
class Solver(object):
    def __init__(self, initial):
        ...
        self.__trace = []
        move_track  = 0 #only for ex2 (in order to keep track of the total number of board positions checked
        ...
        while not puzzle_MinPQ.isEmpty(): #as long as the priority queue is not empty
            current_node = puzzle_MinPQ.delMin() #remove the “best” Node (current_node) from the priority queu
            move_track += 1
            if not current_node.board.solved():  #if the current board is not solved 
                ...
                print ("the total number of board positions checked is:", move_track)
